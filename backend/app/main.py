...
from app.routers import colours, insights, fabrics, users   # ← users must be here
import os
import json
...
# ── CORS ─────────────────────────────────────────────────────
# IMPORTANT: CORSMiddleware must be added BEFORE routers
app.add_middleware(
    CORSMiddleware,
...
def colour_overall():
    ...
    results = [{"colour": c, "count": n, "percentage": round((n / total) * 100, 0)} for c, n in counts.items()]
...
# ── Routers ───────────────────────────────────────────────────
# users.router MUST be included for /auth/signup and /auth/login to work
